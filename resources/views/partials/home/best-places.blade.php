<div class="section best-places" id="best-places">

    <x-title :title="'برترین شهر های ایران'"
        :description="'برترین شهر های ایران که بهترین ویلا ها را میتوانید در آن پیدا کنید'"></x-title>
    <br>
    <br>
    <div class="container">
        <div class="container">
            <div class="row justify-content-center best-cities">
                <div class="col-11 col-md-7 mb-4">
                    <div class="best-city-content">
                        <a href="/search/?name=%20رامسر&type=2">
                            <img src="{{ asset('images/cities/ramsar.jpg') }}" alt="ویلاهای ناب در رامسر">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>رامسر</h3>
                            </div>
                        </a>
                    </div>
                </div>
                <div class="col-11 col-md-5 mb-4">
                    <div class="best-city-content">
                        <a href="/search/?name=تهران&type=2">
                            <img src="{{ asset('images/cities/tehran.jpg') }}" alt="ویلا های عالی در تهران">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>تهران</h3>
                            </div>
                        </a>
                    </div>
                </div>
                <div class="col-11 col-md-3 mb-4">
                    <div class="best-city-content">
                        <a href="/search/?name=سلمان شهر&type=2">
                            <img src="{{ asset('images/cities/motelghoo.jpg') }}" alt="ویلا های فوق العاده در متل قو">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>متل قو</h3>
                            </div>
                        </a>
                    </div>
                </div>

                <br>
                <br>
                <div class="col-11 col-md-3 mb-4">
                    <div class="best-city-content">
                        <a href="/search/?name=شیراز&type=2">
                            <img src="{{ asset('images/cities/shiraz.jpg') }}" alt="ویلاهای بسیار زیبا در شیراز">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>شیراز</h3>
                            </div>
                        </a>
                    </div>
                </div>

                <br>
                <br>
                <br>
                <div class="col-11 col-md-3 mb-4">
                    <div class="best-city-content">
                        <a href="/search/?name=کیش&type=2">
                            <img src="{{ asset('images/cities/kish.jpg') }}" alt="برترین ویلا ها در کیش">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>کیش</h3>
                            </div>
                        </a>
                    </div>
                </div>
                <div class="col-11 col-md-3 mb-4">
                    <a href="/search/?name=رشت&type=2">
                        <div class="best-city-content">
                            <img src="{{ asset('images/cities/rasht.jpg') }}" alt="ویلا های بکر در رشت">
                            <div class="place-effect"></div>
                            <div class="place-info">
                                <h3>رشت</h3>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </div>
</div>
